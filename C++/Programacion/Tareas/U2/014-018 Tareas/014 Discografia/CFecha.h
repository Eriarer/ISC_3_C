...
void CDate::ajuste(int d, int m, int a){
    ...
    dd = (m!=2)? (m<=7)? (m%2 != 0)? ajustarFecha(d, 31):ajustarFecha(d, 30):(m%2 == 0)? ajustarFecha(d, 31):ajustarFecha(d, 30):(aa%4 == 0)? ajustarFecha(d, 29):ajustarFecha(d, 28);
};
void CDate::ajuste(CDate& obj){
    ...
    //primero EXCLUIMOS FEBRERO, SEGUNDO ANTES DE AGOSTO todos los meses impares 31 dias || A PARTIR DE AGOSTO todos los meses pares tienen 31 dias || CASO FEBRERO si es bisiesto tiene 29 dias 
    dd = (obj.getmm()!=2)? (obj.getmm()<=7)? (obj.getmm()%2 != 0)? ajustarFecha(obj.getdd(), 31):ajustarFecha(obj.getdd(), 30):(obj.getmm()%2 == 0)? ajustarFecha(obj.getdd(), 31):ajustarFecha(obj.getdd(), 30):(aa%4 == 0)? ajustarFecha(obj.getdd(), 29):ajustarFecha(obj.getdd(), 28);
};
